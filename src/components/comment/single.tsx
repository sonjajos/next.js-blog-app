...
export default function SingleComment({ data, slug, comments }: SingleCommentProps) {
  ...
  return (
    <div className='flex relative w-full flex-col gap-4'>
      ...
      <p>{data.content}</p>
      <CommentControls slug={slug} postId={data.postId} commentId={data.id} />
      {repliesById && (
        <div className='w-full pl-10'>
          <CommentList data={comments ?? []} slug={slug} psrentId={data.id} />
        </div>
      )}
    </div>
  );
