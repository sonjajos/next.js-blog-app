...
export default function TopicForm({ slug, defaultValues, handleClose }: TopicFormProps) {
  const formRef = useRef<HTMLFormElement | null>(null);
  const [isPending, startTransition] = useTransition();
  const [formState, action] = useActionState(!!slug ? editTopic.bind(null, slug) : createTopic, {
    success: false,
    errors: {
      _form: [],
    },
  ...
  const isDescInvalid = (formState?.errors?.description?.length ?? 0) > 0;
      ...
      <div className='flex flex-col gap-4 p-8 w-full items-center justify-center flex-1'>
        ...
        >
          ...
          <Textarea
            ...
            errorMessage={formState?.errors?.description?.[0] ?? null}
            classNames={{
